...
timezone="Europe/Berlin" #New install timezone setting.
...
mirror_archive="DE" #"" to use the default ubuntu repository. Set to an ISO 3166-1 alpha-2 country code to use a country mirror archive, e.g. "GB". A speed test is run and the fastest archive is selected. Country codes: https://en.wikipedia.org/wiki/ISO_3166-1_alpha-2
...
datapool="scratch" #Non-root drive data pool name.
...
datapoolmount="/$datapool" #Non-root drive data pool mount point in new install.
...
EOF
		...
		landscape-config --silent --account-name="${LANDSCAPE_ACCOUNT_NAME}" --computer-title="${LANDSCAPE_COMPUTER_TITLE}" --tags="" --script-users='nobody,landscape,root' --ping-url="http://${LANDSCAPE_VM_FQDN}/ping" --url="https://${LANDSCAPE_FQDN}/message-system"
